#!/usr/bin/env python3
"""Export an IBM SMI-TED finetune ``.pt`` checkpoint into a MAX model asset dir.

Reads ``MODEL_STATE`` from a finetune checkpoint, writes ``model_weights.safetensors``
plus a patched ``config.json`` with property-serve metadata, and links the SMILES vocab.

Example::

    pixi run export-finetune -- \\
        --checkpoint finetune_ckpts/esol/smi-ted-Light-Finetune_....pt \\
        --task esol
"""
...
DEFAULT_BASE_ASSETS = REPO_ROOT / "model_assets" / "ibm-research_materials.smi-ted"
DEFAULT_OUT_ROOT = REPO_ROOT / "model_assets"
...
def _load_model_state(checkpoint: Path) -> dict[str, torch.Tensor]:
...
def _base_config(base_assets: Path) -> dict[str, Any]:
...
def export_finetune(
    *,
    ...
    net_keys = [k for k in state if k.startswith("net.")]
    if not net_keys:
    ...
    print(f"wrote {weights_path} ({len(state)} tensors, {len(net_keys)} net.*)")

    config = _base_config(base_assets)
    ...
    # Keep a pointer to the source checkpoint for parity scripts.
